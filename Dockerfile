...
RUN wget https://nginx.org/keys/nginx_signing.key \
  ...
  && echo "deb https://nginx.org/packages/mainline/ubuntu/ focal nginx" | tee /etc/apt/sources.list.d/nginx.list \
  && echo "deb-src https://nginx.org/packages/mainline/ubuntu/ focal nginx" | tee /etc/apt/sources.list.d/nginx.src.list \
